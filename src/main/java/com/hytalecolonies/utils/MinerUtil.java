        ...
        for (int dy = 0; dy < size; dy++) {
            for (int dx = 0; dx < size; dx++) {
                for (int dz = 0; dz < size; dz++) {
                    ...
                    nonAir++;
                    DebugLog.fine(DebugCategory.MINER_JOB, "[MinerJob:Scan] Non-air block id=%d at (%d,%d,%d).", blockId, x, y, z);
                    ...
                    if (blockRef != null && chunkStore.getComponent(blockRef, ClaimedBlockComponent.getComponentType()) != null) {
                        claimed++;
                        DebugLog.fine(DebugCategory.MINER_JOB, "[MinerJob:Scan] Block at (%d,%d,%d) is claimed -- skipping.", x, y, z);
                        continue;
                    }
                    DebugLog.fine(DebugCategory.MINER_JOB, "[MinerJob:Scan] Returning block id=%d at (%d,%d,%d).", blockId, x, y, z);
