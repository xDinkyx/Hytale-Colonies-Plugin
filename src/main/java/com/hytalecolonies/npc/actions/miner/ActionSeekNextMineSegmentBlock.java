...
/**
 * Finds the next block to clear in the active mine segment's prefab footprint
 * (blocks that exist in the world where the prefab expects air), then claims it
 * and navigates the miner toward it.
 *
 * <p>Returns {@code null} (sets {@code workAvailable = false}) when:
 * <ul>
...
public class ActionSeekNextMineSegmentBlock extends ActionSeekNextBlockBase {
    ...
    public ActionSeekNextMineSegmentBlock(@Nonnull BuilderActionSeekNextMineSegmentBlock builder,
                                     ...
                                     @Nonnull World world, @Nonnull String npcId)
    {
        ...
        if (ws == null || ws.activeSegmentId == null)
        {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[SeekNextMineSegmentBlock] [%s] No active segment.", npcId);
            return null;
        }
        ...
        {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[SeekNextMineSegmentBlock] [%s] Segment %s not found or has no origin.",
                    npcId, ws.activeSegmentId);
            return null;
        }
        ...
        {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[SeekNextMineSegmentBlock] [%s] Prefab %s not loaded (placeholder?).",
                    npcId, segment.prefabId);
            return null;
        }
        ...
        com.hytalecolonies.ConstructionOrderStore.Entry adapter = new com.hytalecolonies.ConstructionOrderStore.Entry(
        ...
        {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[SeekNextMineSegmentBlock] [%s] Segment %s fully cleared.", npcId, segment.id);
        }
        return next;
    }
    ...
    /**
     * Loads (or returns cached) {@link BlockSelection} for the segment. Returns
     * {@code null} when the prefab is missing or has no blocks defined yet.
     */
    @Nullable
