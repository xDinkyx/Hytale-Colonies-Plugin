...
/**
 * Interactive UI page for adjusting per-category debug log levels and debug visualization toggles at runtime.
 *
 * <p>
 * Each category row shows a cycle {@code TextButton} -- clicking it advances the level through FINE -> INFO -> WARNING -> SEVERE -> OFF -> FINE. Two toggle
 * buttons control debug drawing for colonist paths and tree detection.
 */
    ...
    public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder cmd, @Nonnull UIEventBuilder evt, @Nonnull Store<EntityStore> store)
    {
        ...
        for (DebugCategory category : DebugCategory.values())
        {
            ...
            evt.addEventBinding(CustomUIEventBindingType.Activating,
                                "#" + buttonId,
                                new EventData().append("Action", "cycle_level").append("Category", category.name()),
                                false);
        }
        ...
        evt.addEventBinding(CustomUIEventBindingType.Activating,
                            ...
                            new EventData().append("Action", "toggle_draw").append("DrawTarget", "colonist_paths"),
                            false);
        evt.addEventBinding(CustomUIEventBindingType.Activating,
                            "#DrawTreeDetectionButton",
                            new EventData().append("Action", "toggle_draw").append("DrawTarget", "tree_detection"),
                            false);
        evt.addEventBinding(CustomUIEventBindingType.Activating,
                            "#DrawConstructorOrdersButton",
                            new EventData().append("Action", "toggle_draw").append("DrawTarget", "constructor_orders"),
                            false);
