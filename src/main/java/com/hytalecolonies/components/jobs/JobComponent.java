...
public class JobComponent implements Component<EntityStore>
{
    ...
    public static final BuilderCodec<JobComponent> CODEC =
            BuilderCodec.builder(JobComponent.class, JobComponent::new)
                    ...
                    .add()
                    .append(new KeyedCodec<>("JobState", JobState.CODEC), (o, v) -> o.jobState = v, o -> o.jobState)
                    .add()
                    .build();
    ...
    /// Only updates the field if the caller has the key, which is only obtainable by code in {@link ColonistStateUtil}. This enforces that all state changes go
    /// through the utility method which mirrors to NPC role state.
