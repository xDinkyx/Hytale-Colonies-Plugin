...
/**
 * Determines whether a candidate wood block is the base of a harvestable tree using a 6-connected BFS flood-fill over the wood component.
 *
 * <p>
 * <b>Algorithm:</b>
 * <ol>
 * <li>BFS expands through all face-connected TreeWood blocks reachable from {@code candidatePos}, up to {@value #MAX_WOOD_VISITED} blocks. Exceeding the cap
 * signals a player-built structure or just a really cool old tree (rejected).</li>
 * <li>For each visited wood block, the neighbours are tested for TreeLeaves to build a deduplicated leaf count.</li>
 * <li>The component is accepted as a tree if it meets the minimum wood and leaf thresholds.</li>
 * <li>The BFS naturally identifies the {@code lowestBase} -- the lowest y position in the connected component -- which is not necessarily the block the scanner
 * handed in.</li>
 * </ol>
 */
...
{
    /**
     * Abort BFS if more than this many connected wood blocks are found. Wide-base trees (2Ã—2 or 3Ã—3 trunks) with branches can easily exceed 50 blocks, so a
    ...
    /**
     * Runs the full BFS and returns a {@link TreeCandidate} with the result and diagnostic counts. Useful for debug/logging beyond a boolean answer.
     */
    @Nonnull
    public TreeCandidate evaluate(Vector3i start, World world)
    {
        ...
        while (!queue.isEmpty() && visitedWood.size() <= MAX_WOOD_VISITED)
        {
            ...
            for (int[] d : NEIGHBORS)
            {
                ...
                if (woodKeys.contains(key))
                {
                    ...
                    if (startSpecies != null && !extractSpecies(key).equals(startSpecies))
                        continue;
                    ...
                    if (currentIsBranch && !isBranchBlock(key))
                        continue;
        ...
        boolean isTree = !hitCap && visitedWood.size() >= MIN_WOOD_BLOCKS && visitedLeaf.size() >= MIN_LEAF_BLOCKS;
        ...
        return new TreeCandidate(isTree, reportedBase, visitedWood.size(), visitedLeaf.size(), Collections.unmodifiableSet(visitedWood));
    }

    /**
     * Returns {@code true} if the block key belongs to a branch block (i.e. contains {@code "_Branch_"} -- e.g. {@code Wood_Oak_Branch_Short}).
     */
     ...
     *
     * <p>
     * Block keys follow the pattern {@code Wood_{Species}_{Type}}, where known types are {@code Trunk}, {@code Trunk_Full}, {@code Roots},
     * {@code Branch_Short}, {@code Branch_Long}, and {@code Branch_Corner}. For example, {@code Wood_Oak_Trunk} -> {@code "Wood_Oak"} and
     * {@code Wood_Wisteria_Wild_Branch_Short} -> {@code "Wood_Wisteria_Wild"}.
     *
     * <p>
     * Note: {@code _Trunk_Full} must be checked before {@code _Trunk} because the shorter suffix is a substring of the longer one.
     *
    ...
    private static String extractSpecies(String blockKey)
    {
        ...
        if (blockKey.endsWith("_Trunk_Full"))
            return blockKey.substring(0, blockKey.length() - "_Trunk_Full".length());
        if (blockKey.endsWith("_Trunk"))
            return blockKey.substring(0, blockKey.length() - "_Trunk".length());
        if (blockKey.endsWith("_Roots"))
            return blockKey.substring(0, blockKey.length() - "_Roots".length());
        if (blockKey.endsWith("_Branch_Short"))
            return blockKey.substring(0, blockKey.length() - "_Branch_Short".length());
        if (blockKey.endsWith("_Branch_Long"))
            return blockKey.substring(0, blockKey.length() - "_Branch_Long".length());
        if (blockKey.endsWith("_Branch_Corner"))
            return blockKey.substring(0, blockKey.length() - "_Branch_Corner".length());
        return blockKey; // unknown suffix -- treat the full key as the species
    }

    /**
     * Canonical ordering for the lowest-base tiebreaker: Y ascending, then X ascending, then Z ascending. Using this consistently ensures that a flat
     * multi-block base (all blocks at the same Y) always resolves to the same corner regardless of BFS start order, preventing duplicate block entities across
     * scan invocations.
     */
     ...
     * @param visitedWoodPacked packed positions of every wood block visited -- used by the scanner to skip already-processed blocks
     */
    public record TreeCandidate(boolean isTree, Vector3i base, int woodCount, int leafCount, Set<Long> visitedWoodPacked)
    {
    }
}
