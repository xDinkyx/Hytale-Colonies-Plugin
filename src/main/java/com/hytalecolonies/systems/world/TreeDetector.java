 ...
 *
 * <p>
 * <b>Algorithm (simple vertical stack check):</b>
 * <ol>
 * <li>The block directly below the candidate must be a Soil type, indicating the trunk grew from ground level rather than being mid-structure.</li>
 * <li>The candidate and at least {@value #MIN_STACK_HEIGHT} blocks directly above it must all be TreeWood, confirming a trunk of meaningful height.</li>
 * </ol>
 *
 * <p>
 * This intentionally excludes player-placed constructions because:
 * <ul>
 * <li>Player-built log walls/cabins rarely have a soil block directly beneath a bottom log, and</li>
 * <li>The stack check means a single floating log or a 2-block-high decorative pillar is ignored.</li>
 * </ul>
 *
 * <p>
 * Note: a future upgrade should also check {@code PlacedByInteractionComponent} absence to catch edge cases where a player stacks wood on natural-looking soil.
 */
public class TreeDetector implements ITreeDetector
{
    /**
     * Minimum number of consecutive vertical TreeWood blocks (including the candidate itself) needed to qualify the position as a tree base.
     */
    ...
    /**
     * Returns {@code true} if the block at {@code candidatePos} is the base of a natural, harvestable tree.
     *
     ...
     *
     * @return the block's string ID, or {@code null} if the position is out of bounds, the chunk is not loaded, or the block is air
     */
    @Nullable
