...
/**
 * Pops the next block from the miner's ore-vein queue ({@link MinerJobComponent#oreVeinQueue})
 * and claims it for harvesting. When the queue is empty this action sets {@code workAvailable =
...
public class ActionSeekNextOreVeinBlock extends ActionSeekNextBlockBase {
    ...
    public ActionSeekNextOreVeinBlock(@Nonnull BuilderActionSeekNextOreVeinBlock builder,
                                     ...
                                     @Nonnull World world, @Nonnull String npcId)
    {
        ...
        if (minerJob == null || minerJob.oreVeinQueue.isEmpty())
        {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[SeekNextOreVeinBlock] [%s] Ore vein queue empty.", npcId);
            return null;
        }
        ...
        DebugLog.fine(DebugCategory.MINER_JOB,
                ...
                npcId, head, minerJob.oreVeinQueue.size());
