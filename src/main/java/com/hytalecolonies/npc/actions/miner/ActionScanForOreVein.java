...
/**
 * Scans the 1-block perimeter of the active mine segment for ore veins and
 * populates {@link MinerJobComponent#oreVeinQueue} when one is found.
 * No-op if the queue is already non-empty or there is no active segment.
 */
public class ActionScanForOreVein extends ActionBase {

    public ActionScanForOreVein(@Nonnull BuilderActionScanForOreVein builder,
    ...
    public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role,
        ...
        if (!minerJob.oreVeinQueue.isEmpty()) {
            DebugLog.fine(DebugCategory.MINER_JOB, "[ScanForOreVein] [%s] Queue already non-empty -- skipping scan.",
                    npcId);
            return true;
        }
        ...
        if (ws == null || ws.activeSegmentId == null) {
            DebugLog.fine(DebugCategory.MINER_JOB, "[ScanForOreVein] [%s] No active segment -- skipping ore scan.",
                    npcId);
            return true;
        }
        ...
        if (segment == null || segment.origin == null) {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[ScanForOreVein] [%s] Segment %s not found or missing origin.", npcId, ws.activeSegmentId);
            return true;
        }
        ...
        if (bounds == null) {
            DebugLog.fine(DebugCategory.MINER_JOB,
                    "[ScanForOreVein] [%s] Could not determine segment bounds -- skipping.", npcId);
            return true;
        }
        ...
        if (!vein.isEmpty()) {
            ...
            DebugLog.info(DebugCategory.MINER_JOB,
                    "[ScanForOreVein] [%s] Ore vein of %d blocks found.", npcId, vein.size());
        } else {
    ...
    /**
     * Computes the absolute min/max AABB of the segment's prefab footprint.
     * Uses the cached sorted-block list when available; otherwise loads the prefab.
    ...
    private static Vector3i[] segmentBounds(@Nonnull MineSegmentStore.Entry segment)
    {
        ...
        {
            ConstructionOrderStore.Entry adapter = new ConstructionOrderStore.Entry(
                    segment.id, segment.prefabId, segment.origin);
