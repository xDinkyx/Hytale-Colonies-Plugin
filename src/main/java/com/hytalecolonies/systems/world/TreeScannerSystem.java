 ...
 * scan is handled by {@link WorkstationInitSystem} on workstation load. Block-break / block-place updates are handled reactively by
 * {@link TreeBlockChangeEventSystem}. This system therefore only needs to run infrequently to catch world-generated growth events that have no dedicated event
 * hook.
 */
public class TreeScannerSystem extends DelayedEntitySystem<ChunkStore>
{
    ...
    /**
     * Lazy-initialised cache -- assets are not guaranteed loaded at construction time.
     */
    private Set<String> treeWoodBlockKeys;

    /**
     * Per-chunk wood block count from the last periodic scan. Key: chunk index ({@link ChunkUtil#indexChunk}). Value: total wood-block count across all
     * sections. Chunks whose count matches the cached value are skipped during the periodic scan so unchanged ticks are near-free regardless of world size.
     */
    ...
    /**
     * Scans the chunk grid in {@value #SCAN_RADIUS_CHUNKS}-chunk radius around {@code centerPos} and returns every trunk segment bottom it finds. A segment
     * bottom is a trunk block with a non-wood block directly below it.
     */
    ...
    /**
     * Runs the tree detector over each segment bottom, deduplicating via {@code consumedBlocks}, and returns only the confirmed trees.
     */
    private List<TreeDetectorBFS.TreeCandidate> detectTrees(List<Vector3i> segmentBottoms, World world)
    {
        ...
        for (Vector3i candidate : segmentBottoms)
        {
            ...
            else
            {
                ...
                {
                    confirmedTrees.add(new TreeDetectorBFS.TreeCandidate(true, candidate, 0, 0, Collections.emptySet()));
                }
            }
        }
    ...
    /**
     * Ensures a {@link HarvestableTreeComponent} exists on the block entity at the tree's base position, creating the block entity if necessary.
     *
     * <p>
     * When {@code commandBuffer} is non-null the entity creation / component add is deferred through it (safe inside an ECS tick). When it is {@code null} the
     * store is mutated directly -- only call with a {@code null} buffer from a {@code world.execute()} callback on the world thread.
     *
                                     ...
                                     @Nullable CommandBuffer<ChunkStore> commandBuffer)
    {
        ...
        if (blockType == null)
        {
            DebugLog.warning(DebugCategory.TREE_SCANNER, "[TreeScanner] No block type found at tree base %s -- skipping registration.", base);
            return -2;
        }
        ...
        if (existingRef != null && existingRef.isValid())
        {
            ...
            {
                DebugLog.fine(DebugCategory.TREE_SCANNER, "[TreeScanner] Tree at %s already registered -- skipping.", base);
                return -1;
            }
    ...
    /**
     * Called (from {@link TreeBlockChangeEventSystem}) after a tree-wood block at {@code pos} has been removed from the world.
     *
     * <p>
     * If there is remaining wood below {@code pos} the tree still exists but has lost blocks; re-evaluate via BFS and update (or remove) the
     * {@link HarvestableTreeComponent} at the surviving base. If there is no wood below, the base itself was broken and its block entity (with our component)
     * is auto-removed by the server -- nothing to do.
     *
     * <p>
    ...
    void onTreeWoodBlockRemoved(Vector3i pos, World world, Store<ChunkStore> chunkStore)
    {
        ...
        HarvestableTreeComponent existingComp = chunkStore.getComponent(blockRef, HarvestableTreeComponent.getComponentType());
        ...
        if (result.isTree())
        {
            ...
            DebugLog.info(DebugCategory.TREE_SCANNER, "[TreeScanner] Tree at %s updated -- %d wood blocks remaining after break.", basePos, result.woodCount());
        }
        else
        {
            chunkStore.removeComponent(blockRef, HarvestableTreeComponent.getComponentType());
            DebugLog.info(DebugCategory.TREE_SCANNER, "[TreeScanner] Tree at %s removed -- no valid structure remains.", basePos);
        }
    }

    /**
     * Called (from {@link TreeBlockChangeEventSystem}) after a tree-wood block has been placed at {@code pos}.
     *
     * <p>
     * Walks down to find the segment bottom of the new trunk, runs BFS, and registers a new {@link HarvestableTreeComponent} if the structure qualifies as a
     * tree and is not already registered.
     *
     * <p>
     * Must be called on the world thread (e.g. inside a {@code world.execute()} callback).
     */
    void onTreeWoodBlockAdded(Vector3i pos, World world, Store<ChunkStore> chunkStore)
    {
        ...
        Set<String> woodKeys = getTreeWoodBlockKeys();
        if (!woodKeys.contains(blockType.getId()))
            return;
    ...
    /**
     * Marks the chunk containing the given world position as dirty so the next periodic scan will re-evaluate it regardless of whether its current wood count
     * differs. Should be called by event handlers after they have already handled the immediate change, as a signal to the periodic scan to verify.
     */
