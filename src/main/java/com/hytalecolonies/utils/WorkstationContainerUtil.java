...
/**
 * Generic utility for locating block-based item containers (chests, crates, etc.) that are close to a workstation - within a configurable block radius.
 *
 * <p>
 * Uses the engine's {@link ItemContainerBlock} spatial index for efficient lookup rather than scanning all chunks. This is the same spatial index the server
 * uses internally to track all open containers (see {@code CraftingManager.getContainersAroundBench}).
 *
 * <p>
 * Job-specific delivery systems call {@link #findNearbyContainer} to retrieve the deposit target at runtime. The result is not persisted; the scan is cheap
 * because it only checks blocks already registered with the spatial index.
 */
public final class WorkstationContainerUtil
{
    ...
    /**
     * Returns the world position of the nearest {@link ItemContainerBlock} block within {@code radiusBlocks} of {@code workStationPos}, or {@code null} if
     * none.
     *
