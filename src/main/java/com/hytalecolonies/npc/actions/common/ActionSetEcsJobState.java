...
public class ActionSetEcsJobState extends ActionBase
{
    ...
    public ActionSetEcsJobState(@Nonnull BuilderActionSetEcsJobState builder, @Nonnull BuilderSupport support)
    {
        ...
        catch (IllegalArgumentException e)
        {
            DebugLog.warning(DebugCategory.JOB_SYSTEM,
                             "[SetEcsJobState] Unknown state name '%s' in role JSON -- defaulting to Idle.",
                             builder.getStateName(support));
    ...
    public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store)
    {
        ...
        if (job == null)
        {
            DebugLog.warning(DebugCategory.JOB_SYSTEM, "[SetEcsJobState] [%s] No JobComponent -- cannot set state to %s.", npcId, targetState);
            return true;
        }
