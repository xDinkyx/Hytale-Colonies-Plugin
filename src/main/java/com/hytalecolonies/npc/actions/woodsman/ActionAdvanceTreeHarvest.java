...
/**
 * After the current tree-base block breaks, finds the next adjacent base block and navigates to it, or transitions to {@link JobState#DeliveringItems} when all
...
{
    public ActionAdvanceTreeHarvest(@Nonnull BuilderActionAdvanceTreeHarvest builder, @Nonnull BuilderSupport support)
    {
        super(builder);
    }
    ...
    public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store)
    {
        ...
        {
            DebugLog.warning(DebugCategory.WOODSMAN_JOB, "[AdvanceTreeHarvest] [%s] No job target -- delivering.", npcId);
        ...
        @Nullable
        Vector3i nextBase = allowedTreeTypes != null ? WoodsmanUtil.findNextBaseBlock(treeBase, allowedTreeTypes, world) : null;
        ...
        if (nextBase == null)
        {
            DebugLog.fine(DebugCategory.WOODSMAN_JOB, "[AdvanceTreeHarvest] [%s] No adjacent base at %s -- delivering.", npcId, capturedBase);
        ...
        {
            DebugLog.fine(DebugCategory.WOODSMAN_JOB, "[AdvanceTreeHarvest] [%s] Next base at %s -- traveling.", npcId, nextBase);
            world.execute(() -> ClaimBlockUtil.unclaimBlock(world, capturedBase));
