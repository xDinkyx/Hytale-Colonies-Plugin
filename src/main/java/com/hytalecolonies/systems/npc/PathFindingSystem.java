...
/**
 * Fires when a {@link MoveToTargetComponent} is added to an NPC entity. Writes the target position to the NPC role's "NavTarget" stored position slot (slot 0).
 * The colonist role JSON has a flat ReadPosition + Seek instruction that activates automatically when this slot is set, driving A* pathfinding. The sensor
 * becomes inactive again once the NPC reaches within MinRange of the target.
 */
public class PathFindingSystem extends RefChangeSystem<EntityStore, MoveToTargetComponent>
{
    /**
     * Slot index for the "NavTarget" stored position in the colonist role JSON. Corresponds to the first named position slot used in Template_Colonist.json.
     */
                                 ...
                                 @Nonnull CommandBuffer<EntityStore> commandBuffer)
    {
        ...
        if (npcEntity == null)
        {
            DebugLog.warning(DebugCategory.MOVEMENT, "PathFindingSystem: [%s] entity has no NPCEntity component, cannot navigate.", DebugLog.npcId(ref, store));
            return;
        }
        ...
        {
            showDebugPath(store.getExternalData().getWorld(), transform.getTransform().getPosition(), component.target);
        }
    }
