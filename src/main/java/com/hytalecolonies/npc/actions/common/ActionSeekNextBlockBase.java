...
/**
 * Abstract base for NPC actions that scan for a target block to work at, and sets navigation toward it. Marks {@code workAvailable = false} when the scan
 * returns nothing.
 *
 * <p>
 * Subclasses implement {@link #findNextBlock} for job-specific scans, and may override {@link #preProcess} for one-time setup and {@link #getClaimLabel} for
...
public abstract class ActionSeekNextBlockBase extends ActionBase
{
    ...
    protected ActionSeekNextBlockBase(@Nonnull BuilderActionBase builder, @Nonnull BuilderSupport support)
    {
        super(builder);
    }

    /**
     * Called once per execute before the scan. Override for per-execute workstation-side initialization (e.g., mine origin computation). Default is a no-op.
     */
    protected void preProcess(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull JobComponent job, @Nonnull String npcId) {}

    /**
     * Scans for the next eligible target block. Return {@code null} when none is available (signals exhaustion).
     */
    @Nullable
    protected abstract Vector3i findNextBlock(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull World world, @Nonnull String npcId);
    ...
    public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store)
    {
        ...
        world.execute(() -> {
            ...
            {
                DebugLog.fine(DebugCategory.PERFORMANCE, "[SeekNextBlock] [%s] Claim of %s FAILED (race) -- will retry.", npcId, candidate);
                return;
            }
