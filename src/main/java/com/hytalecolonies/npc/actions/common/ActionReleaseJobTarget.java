...
/**
 * Unclaims the block at {@link JobTargetComponent#targetPosition} and sets the position to {@code null}. {@code SensorJobTargetExists} will return false after
 * this action fires.
 */
public class ActionReleaseJobTarget extends ActionBase
{
    public ActionReleaseJobTarget(@Nonnull BuilderActionReleaseJobTarget builder, @Nonnull BuilderSupport support)
    {
        super(builder);
    }
    ...
    public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store)
    {
        ...
        if (position != null)
        {
            ...
            DebugLog.info(DebugCategory.JOB_SYSTEM, "[ReleaseJobTarget] [%s] Released claim on %s.", npcId, position);
        }
        else
        {
            DebugLog.fine(DebugCategory.JOB_SYSTEM, "[ReleaseJobTarget] [%s] No target to release.", npcId);
        }

        WorkStationComponent workStation = WorkStationUtil.getWorkStation(store, ref);
        if (workStation != null)
        {
            workStation.workAvailable = true;
        }
