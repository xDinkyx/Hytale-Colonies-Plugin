...
/**
 * Periodic safety-net system that runs every 30 seconds to repair two classes of desync:
 *
 * <ol>
 * <li><b>Orphaned block claims</b> -- Any block entity with a {@link ClaimedBlockComponent} whose colonist UUID is no longer valid (colonist has been
 * fired/removed or no longer holds a job target) has its claim released. This covers claims left by crashes, hard removes, or server restarts.</li>
 * <li><b>Colonists with missing workstations</b> -- Any colonist with a {@link JobComponent} whose recorded workstation no longer exists is fired. Primary
 * firing happens in {@link JobAssignmentSystems.WorkStationEntitySystem#onEntityRemove}; this is a catch-all backstop.</li>
 * </ol>
 */
public class ColonistCleanupSystem extends DelayedSystem<ChunkStore>
{
    ...
    private static void releaseOrphanedClaims(@Nonnull List<Ref<ChunkStore>> orphanedRefs)
    {
        ...
        if (cleared > 0)
        {
            DebugLog.info(DebugCategory.JOB_ASSIGNMENT, "[ColonistCleanup] Released %d orphaned block claim(s).", cleared);
        }
    }

    private static void fireColonistsWithMissingWorkstation(@Nonnull List<Ref<EntityStore>> orphans, @Nonnull EntityStore entityStore)
    {
