...
{
    // ===== Codec =====
    public static final BuilderCodec<ColonistComponent> CODEC =
            BuilderCodec.builder(ColonistComponent.class, ColonistComponent::new)
                    .append(new KeyedCodec<>("ColonyId", Codec.STRING), (c, v) -> c.colonyId = v, c -> c.colonyId)
                    .add()
                    .append(new KeyedCodec<>("ColonistName", Codec.STRING), (c, v) -> c.colonistName = v, c -> c.colonistName)
                    .add()
                    .append(new KeyedCodec<>("ColonistLevel", Codec.INTEGER), (c, v) -> c.colonistLevel = v, c -> c.colonistLevel)
                    .add()
                    .build();
    ...
    // ===== Component Type =====
    public static ComponentType<EntityStore, ColonistComponent> getComponentType()
    {
        return HytaleColoniesPlugin.getInstance().getColonistComponentType();
    }
