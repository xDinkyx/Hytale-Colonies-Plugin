...
/**
 * Generic job-target component. Present on a colonist during an active job cycle (from claiming a target through returning home).
 *
 * <p>
 * {@code targetPosition} is the world position the colonist should travel to for their task. It is set to {@code null} once the task finishes and the colonist
 * begins heading home, so that {@link com.hytalecolonies.systems.jobs.JobAssignmentSystems.StaleMarkCleanupSystem} does not treat an already-released claim as
 * still active.
 *
 * <p>
 * {@code lastKnownPosition} and {@code stuckTicks} are transient stuck-detection fields used by {@link com.hytalecolonies.systems.jobs.ColonistMovementSystem}.
