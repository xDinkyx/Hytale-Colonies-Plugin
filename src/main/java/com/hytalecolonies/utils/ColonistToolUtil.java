 ...
 * <p>
 * A tool's spec for a gather type must pass two checks (mirroring {@code BlockHarvestUtils.getSpecPowerDamageBlock}):
 * <ol>
 * <li><strong>Power &gt; unarmed baseline</strong> -- the spec must beat the power in {@code Server/Item/Unarmed/Gathering/&lt;GatherType&gt;.json}, so
 * equipping the tool is actually an improvement. A crude hatchet's {@code Soils: 0.05} loses to unarmed {@code Soils: 0.10} and is therefore excluded.</li>
 * <li><strong>Quality &ge; required quality</strong> -- the block's {@code Breaking.Quality} sets a minimum tier. A tier-3 Rocks block requires an iron pickaxe
 * (quality 3); a crude pickaxe (quality 0) cannot break it even though it technically has a {@code Rocks} spec.</li>
 * </ol>
 * <p>
 * Both checks are purely data-driven -- no magic numbers here.
 */
public final class ColonistToolUtil
{
    ...
    /**
     * Returns the power value of {@code tool}'s spec for {@code gatherType}, or {@code 0} if the tool has no spec for it.
     */
    ...
    /**
     * Returns {@code true} if {@code tool} can break a block with {@code gatherType} at the given {@code requiredQuality} tier. Mirrors the logic in
     * {@code BlockHarvestUtils.getSpecPowerDamageBlock}: the spec's {@code GatherType} must match and its {@code Quality} must be &ge; {@code requiredQuality}.
     * There is no unarmed-power floor -- any power value is valid as long as the quality tier is met.
     *
     * @param requiredQuality the {@code Breaking.Quality} value from the block's JSON (0 = no tier requirement).
     */
    public static boolean toolSupportsGatherType(@Nullable ItemTool tool, @Nonnull String gatherType, int requiredQuality)
    {
        if (tool == null || tool.getSpecs() == null)
            return false;
        for (ItemToolSpec spec : tool.getSpecs())
     ...
     *
     * @param container The {@link ItemContainer} the tool currently lives in ({@code hotbar} or {@code storage/backpack}).
     * @param slot      The slot index within that container.
     * @param power     The tool's power for the requested gather type.
     * @param inHotbar  {@code true} when {@code container} is the hotbar -- the tool can be equipped with a simple slot switch.
     */
    public record ToolMatch(@Nonnull ItemContainer container, short slot, float power, boolean inHotbar)
    {
    }

    /**
     * Searches the colonist's entire inventory (hotbar -> storage -> backpack) for the <em>best</em> tool for the block described by {@code breaking} and
     * returns a {@link ToolMatch}, or {@code null} if no suitable tool exists.
     *
     * <p>
     * "Suitable" means the spec's {@code GatherType} matches and its {@code Quality} is &ge; the block's required quality. Among suitable tools the one with
     * the highest power wins. Hotbar is preferred over storage when power is equal.
     */
    @Nullable
    public static ToolMatch findBestToolInInventory(@Nonnull Inventory inventory, @Nonnull BlockBreakingDropType breaking)
    {
    ...
    /**
     * Returns {@code true} if the colonist has any suitable tool for the given block's breaking requirements anywhere in their inventory.
     */
    public static boolean hasToolForBlock(@Nonnull Inventory inventory, @Nonnull BlockBreakingDropType breaking)
    {
        return findBestToolInInventory(inventory, breaking) != null;
    }

    /**
     * Ensures the best available tool for the given block's breaking requirements is the colonist's active hotbar item. No-ops if the current item in hand is
     * already the best.
     *
    ...
    /**
     * Returns {@code true} if the colonist has any tool in their inventory capable of harvesting a block with {@code gatherType} at {@code requiredQuality}
     * tier. Used for "wait at workstation" checks before a job begins.
     */
    public static boolean hasToolForGatherType(@Nonnull Inventory inventory, @Nonnull String gatherType, int requiredQuality)
    {
        return findBestToolForGatherType(inventory, gatherType, requiredQuality) != null;
    }

    /**
     * Equips the best tool in the colonist's inventory for {@code gatherType} at the given quality tier. Companion to {@link #equipBestToolForBlock} for
     * callers that know the gather type directly (e.g. NPC actions) rather than from a block position.
     *
     * @return {@code true} if a suitable tool was equipped (or was already in hand), {@code false} if no suitable tool exists in the inventory.
     */
    ...
    betterMatch(@Nullable ToolMatch current, @Nullable ItemContainer container, @Nonnull String gatherType, int requiredQuality, boolean inHotbar)
    {
        if (container == null)
            return current;
