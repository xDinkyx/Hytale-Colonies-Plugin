...
/**
 * Registry of {@link ChunkStore} component types that implement {@link IClaimableBlock}.
 *
 * <p>
 * Register each claimable block component type on plugin startup (before any colonist is assigned). The general unclaim utility ({@link #unclaimTargetBlock})
 * then works for any job type without needing to know which specific component marks the block.
 *
 * <p>
 * Example registration in the plugin:
 *
 * <pre>{@code
...
{
    private static final List<ComponentType<ChunkStore, ? extends IClaimableBlock>> registeredTypes = new ArrayList<>();

    private ClaimedBlockRegistry() {}

    /**
     * Registers a claimable block component type. Must be called during plugin setup before any colonist is assigned a job.
     */
    ...
    /**
     * Returns an unmodifiable view of all registered claimable component types. Used by the cleanup system to sweep orphaned marks.
     */
     ...
     *
     * <p>
     * Reads the target position from the colonist's {@link JobTargetComponent}, looks up the block entity, then calls {@link IClaimableBlock#unclaim()} on
     * whichever registered component type is present. Safe to call when no block is claimed (no-op).
     */
