 ...
 * <h3>Thread safety</h3>
 * <ul>
 * <li>{@link #unclaimBlock} and {@link #unclaimByColonist} must be called from the world thread (e.g. inside a {@code world.execute()} callback) since they
 * mutate ChunkStore data directly.</li>
 * <li>Checking whether a block is already claimed from inside an EntityStore tick is safe because {@code store.getComponent()} is a read-only access.</li>
 * </ul>
 *
 * <h3>Multi-block support</h3> {@link #resolveCanonicalPosition} applies the filler-block logic from the engine (same as used by
 * {@code BlockEntityInfoCommand}) to map any position within a large multi-block structure to the canonical position that hosts the block entity.
 */
public final class ClaimBlockUtil
{
    ...
    /**
     * Snapshots and clears {@link ConstructorJobComponent#pendingBuildQueue}, then unclaims all captured positions on the world thread. Does nothing if the
     ...
     *
     * <p>
     * Works for <em>any</em> block, whether or not it has a pre-existing block entity:
     * <ul>
     * <li>If a block entity already exists (e.g. tree with {@link com.hytalecolonies.components.world.HarvestableTreeComponent}), the
     * {@link ClaimedBlockComponent} is attached to that entity and removed on unclaim without affecting the entity itself.</li>
     * <li>If no block entity exists (e.g. a plain stone block in a mine), a minimal entity ({@code BlockStateInfo} + {@code ClaimedBlockComponent}) is created
     * at the position. That entity is destroyed completely on unclaim.</li>
     * </ul>
     *
     * <p>
     * Internally resolves filler-block indirection so multi-block structures are always claimed at their canonical position.
     *
     ...
     * @return {@code true} if the claim was placed; {@code false} if the chunk is not loaded or the block is already claimed
     */
    public static boolean claimBlock(World world, Vector3i position, UUID claimedByUuid, String claimType)
    {
        ...
        if (canonical == null)
        {
            DebugLog.fine(DebugCategory.CLAIM_SYSTEM, "[Claim] claimBlock(%s) -- chunk not loaded, cannot claim.", position);
            return false;
        }
        ...
        if (blockRef != null && blockRef.isValid())
        {
            ...
            if (existing != null)
            {
                DebugLog.fine(DebugCategory.CLAIM_SYSTEM,
                              ...
                              canonical,
                              existing.getClaimedByUuid(),
                              existing.getClaimType());
                return false;
            }
            chunkStore.putComponent(blockRef, ClaimedBlockComponent.getComponentType(), new ClaimedBlockComponent(claimedByUuid, claimType));
            DebugLog.info(DebugCategory.CLAIM_SYSTEM, "[Claim] Claimed block at %s by %s (type=%s).", canonical, claimedByUuid, claimType);
        }
        else
        {
     ...
     * workstations).
     *
     * @return the canonical world position, or {@code null} if the chunk is not loaded
     */
    @Nullable
     ...
     *
     * <p>
     * Cleanup of claim-only entities (those that have no other components) is handled automatically by
     * {@link com.hytalecolonies.systems.world.ClaimedBlockCleanupSystem}, which watches for {@code ClaimedBlockComponent} removal and destroys the entity if
     * nothing else uses it.
     *
     * <p>
     * Safe to call when no claim exists (no-op). Must be called from the world thread (e.g. inside {@code world.execute()}).
     */
    public static void unclaimBlock(World world, Vector3i position)
    {
        ...
        if (canonical == null)
        {
            DebugLog.fine(DebugCategory.CLAIM_SYSTEM, "[Claim] unclaimBlock(%s) -- chunk not loaded, nothing to remove.", position);
            return;
        }
        ...
        if (blockRef == null || !blockRef.isValid())
        {
            DebugLog.fine(DebugCategory.CLAIM_SYSTEM, "[Claim] unclaimBlock(%s) -- no block entity, nothing to remove.", canonical);
            return;
        }
    ...
    /**
     * Reads the colonist's {@link JobTargetComponent} and releases the claim on the targeted block, if any.
     *
     * <p>
     * Must be called from the world thread (e.g. inside a {@code world.execute()} callback).
     */
    ...
    /**
     * Claims the base position <em>and</em> all filler cells the block occupies at the given rotation. If any cell is already claimed or unavailable, all
     * claims made in this call are rolled back and {@code false} is returned. Must be called on the world thread.
     */
    public static boolean claimBlockAndFillers(World world, int wx, int wy, int wz, BlockType blockType, int rotation, UUID claimedByUuid, String claimType)
    {
    ...
    /**
     * Unlaims the base position and all filler cells the block occupies. If {@code blockType} or {@code rotation} are unavailable, only the base position is
     * unclaimed. Must be called on the world thread.
     */
    public static void unclaimBlockAndFillers(World world, Vector3i basePos, @Nullable BlockType blockType, int rotation)
    {
