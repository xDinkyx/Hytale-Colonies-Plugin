...
/**
 * Watches for {@link ClaimedBlockComponent} removal and automatically destroys block entities that were created solely to host a claim.
 *
 * <p>
 * When a block entity has only {@code BlockStateInfo} remaining after {@code ClaimedBlockComponent} is stripped (archetype length == 1), the entity serves no
 * purpose and is queued for removal via the {@link CommandBuffer}.
 *
 * <p>
 * When a block is broken by a player the engine already calls {@code removeEntity} on the block entity, which cascades {@code onComponentRemoved} for every
 * component including {@code ClaimedBlockComponent}. In that path the ref is no longer valid when this callback fires, so the guard at the top of
 * {@link #onComponentRemoved} prevents a double-destroy.
 */
                                   ...
                                   @Nonnull CommandBuffer<ChunkStore> commandBuffer)
    {
        ...
        if (archetype.length() <= 1)
        {
            DebugLog.info(DebugCategory.CLAIM_SYSTEM,
                          ...
                          removed.getClaimedByUuid(),
                          removed.getClaimType());
